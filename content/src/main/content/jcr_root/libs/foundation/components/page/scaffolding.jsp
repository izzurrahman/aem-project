...
%><cq:defineObjects/><%
    ...
    if (scaffoldPath == null || scaffoldPath.length() == 0) {
        // use default
        scaffoldPath = "/libs/wcm/scaffolding/content/scaffolding";
    }
    ...
    if (resourceResolver.getResource(scaffoldPath + "/cq:dialog") != null) {
        try {
            ...
            %><sling:include path="<%= "/libs/wcm/core/content/editor/scaffoldhost.html/" + xssAPI.getValidHref(scaffoldPath) %>" /><%
        } finally {
        ...
        %><cq:include path="<%= xssAPI.getValidHref(scaffoldPath) + ".html" %>" resourceType="wcm/scaffolding/components/scaffolding" /><%
    }
