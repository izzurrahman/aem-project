...
%><center>
  <form action="${currentPagePath}.html">
  ...
  <c:otherwise>
    ...
    <div class="searchRight">
          <c:if test="${fn:length(trends.queries) > 0}">
              ...
              <div class="searchTrends">
                  <c:forEach var="query" items="${trends.queries}">
                      <a href="<c:url value="${currentPagePath}.html"><c:param name="q" value="${query.query}"/></c:url>"><span style="font-size:${query.size}px"><c:out value="${query.query}"/></span></a>
                  </c:forEach>
              </div>
          </c:if>
              ...
              <c:forEach var="bucket" items="${result.facets.languages.buckets}">
                  ...
                  <c:choose>
                      <c:when test="${param.language != null}">
                        ${label} (${bucket.count}) - <a title="filter results" href="<c:url value="${currentPagePath}.html"><c:param name="q" value="${escapedQueryForHref}"/></c:url>">remove filter</a>
                      </c:when>
                      <c:otherwise>
                          <a title="filter results" href="
                            <c:url value="${currentPagePath}.html">
              ...
              <c:forEach var="bucket" items="${result.facets.lastModified.buckets}">
                  <c:choose>
                      <c:when test="${param.from == bucket.from && param.to == bucket.to}">
                        ${bucket.value} (${bucket.count}) - <a title="filter results" href="<c:url value="${currentPagePath}.html"><c:param name="q" value="${escapedQueryForHref}"/></c:url>">remove filter</a>
                      </c:when>
                      <c:otherwise>
                          <a title="filter results" href="
                            <c:url value="${currentPagePath}.html">
      ...
      <c:forEach var="hit" items="${result.hits}" varStatus="status">
        ...
        <a href="<c:url value="${hit.URL}" context="/"/>" onclick="trackSelectedResult(this, ${status.index + 1})">${hit.title}</a>
        <div>${hit.excerpt}</div>
      ...
      <c:if test="${fn:length(result.resultPages) > 1}">
        ...
        %><c:choose>
            <c:when test="${page.currentPage}">${searchPageIndex}</c:when>
            <c:otherwise>
